// Win32Integration.cpp : Defines the entry point for the application.
//
                     ...
                     int       nCmdShow)
{
	...
	LoadString(hInstance, IDC_WIN32INTEGRATION, szWindowClass, MAX_LOADSTRING);
	...
	ApplicationKit::init( __argc, __argv );

	//COM stuff for VCF, happens automaticallin in Application peer...
	OleInitialize(NULL);
	...
	hAccelTable = LoadAccelerators(hInstance, (LPCTSTR)IDC_WIN32INTEGRATION);
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
	...
	wcex.hIcon			= LoadIcon(hInstance, (LPCTSTR)IDI_WIN32INTEGRATION);
	wcex.hCursor		= LoadCursor(NULL, IDC_ARROW);
	wcex.hbrBackground	= (HBRUSH)(COLOR_WINDOW+1);
	wcex.lpszMenuName	= (LPCSTR)IDC_WIN32INTEGRATION;
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch (message) 
	{
		...
		case WM_PAINT: 
			...
			GraphicsContext gc(ps.hdc);

			Font* font = gc.getCurrentFont();
			font->setBold( true );
			font->setColor( &Color("red") );
			font->setPointSize( 22 );
			gc.textAt( 10, 10, "Hello from the VCF!!!" );
