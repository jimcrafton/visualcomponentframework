...
//----------------------------------------------------------------------------
//
// classes span_rgb24_image*
//
//----------------------------------------------------------------------------

#ifndef AGG_SPAN_RGB24_IMAGE_INCLUDED
#define AGG_SPAN_RGB24_IMAGE_INCLUDED
...
namespace agg
{
    ...
    //--------------------------------------------------------------------
    template<class Attribute> class span_rgb24_image_simple
    {
    public:
            ...
            do
            {
                   ...
                   x <= maxx && y <= maxy) 
                {
                    ...
                    int alpha = gradient_alpha.calculate(
                        color_alpha->calculate(*covers++, fg_r, fg_g, fg_b));
                    ...
                    else
                    {
                        ...
                        int bk_g = bk_ptr[1];
                        int bk_b = bk_ptr[2];
                        *bk_ptr++ = (((fg_r - bk_r) * alpha) + (bk_r << 8) + 0x80) >> 8;
                        *bk_ptr++ = (((fg_g - bk_g) * alpha) + (bk_g << 8) + 0x80) >> 8;
                        *bk_ptr++ = (((fg_b - bk_b) * alpha) + (bk_b << 8) + 0x80) >> 8;
                    }
                }
                else
                {
    ...
    //--------------------------------------------------------------------
    template<class Attribute> class span_rgb24_image_bilinear
    {
    public:
        typedef Attribute attr_type;
                           ...
                           const Attribute& attr)
        {
            ...
            do
            {
                   ...
                   x_lr <= maxx && y_lr <= maxy) 
                {
                    fg_r = 
                    fg_g = 
                    fg_b = image_subpixel_size * image_subpixel_size - 1;
    ...
    //--------------------------------------------------------------------
    template<class Attribute> class span_rgb24_image
    {
    public:
        typedef Attribute attr_type;
                           ...
                           const Attribute& attr)
        {
            ...
            typename Attribute::gradient_alpha_type gradient_alpha(*attr.m_gradient_alpha, x, y, count);
            const rendering_buffer* src = attr.m_src;
            int stride = src->row_bytes() - dimension * 3;
            ...
            do
            {
                ...
                if(x_lr >= -start && y_lr >= -start &&
                   x_lr <= maxx   && y_lr <= maxy) 
                {
                    fg_r = fg_g = fg_b = image_filter_size / 2;
