...
int CommandLine::SplitLine(int argc, char **argv)
{
   ...
   for (int i = 1; i < argc; i++)
   {
      ...
      if (IsSwitch(argv[i]))
      {
         ...
         pair<CommandLine::iterator, bool> res = insert(CommandLine::value_type(curParam, cmd));
      ...
      else
      {
         ...
         // ...let's add it
 	      CommandLine::iterator theIterator;
...
bool CommandLine::IsSwitch(const char *pParam)
{
   if (pParam==NULL)
      return false;
...
/*------------------------------------------------------
   bool CommandLine::HasSwitch(const char *pSwitch)
...
/*------------------------------------------------------
   ...
   StringType CommandLine::GetSafeArgument(const char *pSwitch, int iIdx, const char *pDefault)
...
/*------------------------------------------------------
   ...
   StringType CommandLine::GetArgument(const char *pSwitch, int iIdx)
...
StringType CommandLine::GetArgument(const char *pSwitch, int iIdx)
{
   if (HasSwitch(pSwitch))
   {
      ...
      theIterator = find(pSwitch);
	   if (theIterator!=end())
      {
         if ((int)(*theIterator).second.strings_.size() > iIdx)
         {
            return (*theIterator).second.strings_[iIdx];
         }
      }
   }
...
/*------------------------------------------------------
   int CommandLine::GetArgumentCount(const char *pSwitch)
...
int CommandLine::GetArgumentCount(const char *pSwitch)
{
   ...
   if (HasSwitch(pSwitch))
   {
      ...
      theIterator = find(pSwitch);
	   if (theIterator!=end())
      {
         iArgumentCount = (*theIterator).second.strings_.size();
      }
   }
