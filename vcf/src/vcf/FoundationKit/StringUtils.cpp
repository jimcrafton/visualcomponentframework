...
String StringUtils::eraseRightOfChar( const String& s, const VCFChar& ch, const bool& included/*=false*/, const int& count/*=1*/ )
{
	...
	if ( String::npos != pos ) {
		if ( included ) {
			str = str.erase( pos, str.size() - pos );
		} else {
			str = str.erase( pos + 1, str.size() - pos - 1 );
		}
	}
	...
	while ( (p-start) < size ) {
		if ( ('%' == *p) || (hashCharFound) ) {
			...
			switch ( *p ) {
				...
				//	%% - Percent sign
				case '%' : {
					result = result.append( current, (p-current) -formatArgCount );
					result += "%";
...
String StringUtils::convertFormatString( const String& formattedString )
{
		...
		while ( pos != String::npos ) {
			result = result.erase( pos, lsDirective.length() );
			result = result.insert( pos, "%s" );
