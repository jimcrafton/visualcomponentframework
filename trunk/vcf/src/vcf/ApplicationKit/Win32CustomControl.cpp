...
OSHandleID Win32CustomControl::getHandleID()
{
	OSHandleID result = 0;
	result = (OSHandleID)this->wndHandle_;
	return result;
}
...
{
	bool result = false;
	LONG_PTR style = GetWindowLongPtr( this->wndHandle_, GWL_STYLE );
...
{
	uint32 result = 0;
	result = (uint32) GetWindowLongPtr( this->wndHandle_, GWL_STYLE );
	return result;
}
