...
*/
class APPLICATIONKIT_API NativeEvent : public Event {
public:
	NativeEvent( Control* source );

	NativeEvent( const NativeEvent& rhs );

	virtual ~NativeEvent();

	NativeEvent& operator=( const NativeEvent& rhs );

	virtual Object* clone( bool deep=false );

	bool passOnToDefaulthandler;

	/**
	The native data for the event
	Under Win32 this will be a Win32MSG struct.
	*/
	void* nativeOSEventData;
};
...
typedef Delegate1<NativeEvent*> NativeDelegate;
...
public:
	...
	enum ControlState {
		...
		csAllowNativeEventHandling	= 0x00010000,
	...
	*/
	...
	/**
	This occurs for any other non specific control event, for custom processing.
	If there's no attached handlers in the delegate, then nothing is done
	@event NativeEvent
	@eventtype Control::BEFORE_POPUP_MENU
	*/
	DELEGATE(NativeDelegate,NativeEventRaised);
	...
	*/
	...
	bool allowsNativeEventHandling() {
		return (controlState_ & Control::csAllowNativeEventHandling) ? true : false;
	};

	/**
	Sets whether or not the control should event allow passing along native events to the event delegate.
	Added as another way to optimize the handling here. By default it's turned off, you need to 
	manually turn this for specific controls. 
	*/
	void setAllowsNativeEventHandling( const bool& allowNativeEventHandling );
...
{		
	*this = rhs;
	...
	inline NativeEvent::NativeEvent( Control* source ) : 
			Event(source),nativeOSEventData(NULL),passOnToDefaulthandler(true)
	{
	}

	inline NativeEvent::NativeEvent( const NativeEvent& rhs ): 
		Event( rhs ), nativeOSEventData(NULL),passOnToDefaulthandler(true) 
	{

	}

	inline NativeEvent::~NativeEvent() 
	{
	}

	inline NativeEvent& NativeEvent::operator=( const NativeEvent& rhs ) 
	{
		Event::operator =( rhs );
		nativeOSEventData = NULL;
		passOnToDefaulthandler = rhs.passOnToDefaulthandler;
		return *this;
	}

	inline Object* NativeEvent::clone( bool deep ) 
	{
