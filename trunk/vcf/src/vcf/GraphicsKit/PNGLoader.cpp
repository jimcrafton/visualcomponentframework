...
struct PNGLoaderIOInfo {
	PNGLoaderIOInfo():infoPtr(NULL),inStream(NULL),outStream(NULL){}

	png_infop infoPtr;
	VCF::InputStream* inStream;
	VCF::OutputStream* outStream;	
};
...
static void
pngLoader_ReadProc( png_struct * pngInfo, png_byte* data, png_size_t size) 
{
	PNGLoaderIOInfo* ioInfo = (PNGLoaderIOInfo*) ::png_get_io_ptr( pngInfo );

	if ( NULL != ioInfo ) {
		ioInfo->inStream->read( data, size );
	}
}

static void
pngLoader_WriteProc( png_struct * pngInfo, png_byte *data, png_size_t size) 
{
	PNGLoaderIOInfo* ioInfo = (PNGLoaderIOInfo*) ::png_get_io_ptr( pngInfo );

	if ( NULL != ioInfo ) {
		ioInfo->outStream->write( data, size );
	}
}
...
Image* PNGLoader::loadImageFromFile( const String& fileName )
{
	...
	Image* result = loadImageFromStream( &fis );
...
//	int palette_entries;
	...
	// init the IO
	ioInfo.infoPtr = info_ptr;
	png_set_write_fn(png_ptr, &ioInfo, pngLoader_WriteProc, pngLoader_FlushProc);
...
Image* PNGLoader::loadImageFromBytes( const unsigned char* imageData, const uint64& dataLength )
{
	Image* result = NULL;
...
Image* PNGLoader::loadImageFromStream( InputStream* stream )
{
	Image* result = NULL;

	PNGLoaderIOInfo ioInfo;
	ioInfo.inStream = stream;
	...
	stream->read( png_check, PNG_BYTES_TO_CHECK );
	...
	png_set_read_fn(png_ptr, &ioInfo, pngLoader_ReadProc);
