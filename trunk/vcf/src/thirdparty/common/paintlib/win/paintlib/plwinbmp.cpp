...
{
  // bdelmee code change
  int nBytes = m_Size.x*GetBitsPerPixel() / 8;
  if (GetBitsPerPixel() == 1 && m_Size.x % 8)
    ++nBytes;
...
void PLWinBmp::CreateFromHBitmap(HBITMAP hBitMap)
{
  ...
  internalCreate(pBIH->biWidth, pBIH->biHeight, pixelFormatFromBMI(pBIH));
...
void PLWinBmp::internalCreate (LONG Width, LONG Height, const PLPixelFormat& pf)
...
// Assumes that no memory is allocated before the call.
{
  ...
  initLocals (m_pBMI->biWidth, m_pBMI->biHeight, pixelFormatFromBMI(m_pBMI));
...
const PLPixelFormat& PLWinBmp::pixelFormatFromBMI(const BITMAPINFOHEADER * pBMI) const
{
  switch(pBMI->biBitCount) 
  {
    case 1:
      return PLPixelFormat::L1;
    case 8:
      return PLPixelFormat::I8;
    case 16:
      return PLPixelFormat::X1R5G5B5;
    case 24:
...
|
|      $Log: plwinbmp.cpp,v $
|      Revision 1.5  2004/09/11 12:41:37  uzadow
|      removed plstdpch.h
|
|      Revision 1.4  2004/09/09 16:52:50  artcom
|      refactored PixelFormat
|
|      Revision 1.3  2004/06/19 17:04:22  uzadow
