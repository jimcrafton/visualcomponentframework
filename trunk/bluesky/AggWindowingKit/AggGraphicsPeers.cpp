...
AggFont::AggFont( const String& fontName )
{
	init();
}

AggFont::AggFont( const String& fontName, const double& pointSize )
{
	init();
}
...
OSHandleID AggFont::getFontHandleID() 
{
	return NULL;
}

double AggFont::getPixelSize() 
{
	return 0.0;
}

void AggFont::setPixelSize( const double pixelSize )
{

}

void AggFont::setAttributes( const double& pointSize, const bool& bold, const bool& italic,
...
Size AggFont::getTextSize( const String& text )
{
	AggGraphicsToolkit* tk = (AggGraphicsToolkit*)GraphicsToolkit::internal_getDefaultGraphicsToolkit();
	return tk->getTextSize( text, (Font*) this );
}

void AggFont::updateLocaleSettings()
{

}

GlyphCollection* AggFont::getGlyphCollection( const String& text )  
{
	return NULL;
}

bool AggFont::isEqual( Object* object )
{
	return false;
}
...
AggRGBAImage::AggRGBAImage()
{

}

AggRGBAImage::AggRGBAImage( const String& fileName )
{

}

AggRGBAImage::AggRGBAImage( const uint32& width, const uint32& height )
{

}

AggRGBAImage::AggRGBAImage( GraphicsContext* context, Rect* rect )
{
...
void AggRGBAImage::setSize( const uint32 & width, const uint32 & height )
{

}

void AggRGBAImage::beginDrawing()
{

}

void AggRGBAImage::finishedDrawing()
{
...
AggGrayScaleImage::AggGrayScaleImage()
{

}

AggGrayScaleImage::AggGrayScaleImage( const String& fileName )
{

}

AggGrayScaleImage::AggGrayScaleImage( const uint32& width, const uint32& height )
{

}

AggGrayScaleImage::AggGrayScaleImage( GraphicsContext* context, Rect* rect )
{
...
void AggGrayScaleImage::setSize( const uint32 & width, const uint32 & height )
{

}

void AggGrayScaleImage::beginDrawing()
{

}

void AggGrayScaleImage::finishedDrawing()
{
...
void AggContextPeer::checkHandle()
{
	//no-op
}

void AggContextPeer::releaseHandle()
{
	//no-op
}
...
void AggContextPeer::clearGlyphs()
{
	std::vector<CachedGlyph*>::iterator it = cachedFontGlyphs_.begin();
	while ( it != cachedFontGlyphs_.end() ) {
		if ( NULL != *it ) {
...
const agg::glyph_cache* AggContextPeer::glyph( int character, double& x, double& y )
{
	CachedGlyph* cg = NULL;
...
bool AggContextPeer::prepareForDrawing( int32 drawingOperation )
{
	resetPath();
...
/*
			...
			FcPattern* pattern = AggGraphicsToolkit::getFontPatternForFont( fontPeer );
...
void AggContextPeer::finishedDrawing( int32 drawingOperation )
{
...
void AggContextPeer::renderScanlinesSolid( agg::rasterizer_scanline_aa<>& rasterizer, const agg::rgba& color )
{
...
void AggContextPeer::setClippingPath( Path* clippingPath )
{
	//LinuxDebugUtils::FunctionNotImplemented(__FUNCTION__);
}

void AggContextPeer::setClippingRect( Rect* clipRect )
{
	if ( NULL == clipRect ) {
...
void AggContextPeer::setTextAlignment( const bool& alignTobaseline )
{
	//LinuxDebugUtils::FunctionNotImplemented(__FUNCTION__);
}

bool AggContextPeer::isTextAlignedToBaseline()
{
	return true;
}

void AggContextPeer::internal_setGamma( double gamma )
{
...
void AggContextPeer::renderLine( const std::vector<GlyphInfo>& glyphs, size_t lastGlyphPos, const Size& currentLineSz,
...
void AggContextPeer::textAt( const Rect& bounds, const String & text, const int32& drawOptions )
{
	VCF_ASSERT( fonts_->fontLoaded );
...
Size AggContextPeer::getTextSize( const String& text )
{
	Size result;
...
double AggContextPeer::getTextWidth( const String& text )
{
...
double AggContextPeer::getTextHeight( const String& text )
{
	Size sz = getTextSize( text );
...
void AggContextPeer::rectangle( const double & x1, const double & y1,
...
void AggContextPeer::roundRect( const double & x1, const double & y1,
								const double & x2, const double & y2,
...
void AggContextPeer::ellipse( const double & x1, const double & y1,
							  const double & x2, const double & y2 )
{
	//LinuxDebugUtils::FunctionNotImplemented(__FUNCTION__);
	//Not needed - implemented in GraphicsContext
...
void AggContextPeer::arc( const double & x1, const double & y1,
						  const double & x2, const double & y2,
...
void AggContextPeer::polyline( const std::vector<Point>& pts )
{
	if ( pts.empty() ) {
		return;
	}
...
void AggContextPeer::curve( const double & x1, const double & y1, const double & x2, const double & y2,
...
void AggContextPeer::moveTo( const double & x, const double & y )
{
	currentPath_.move_to( x, y );
}

void AggContextPeer::lineTo( const double & x, const double & y )
{
	currentPath_.line_to( x, y );
}

void AggContextPeer::closePath()
{
	currentPath_.close_polygon();
}

void AggContextPeer::setOrigin( const double& x, const double& y )
{
	origin_.x_ = x;
	origin_.y_ = y;
}

Point AggContextPeer::getOrigin()
{
	return origin_;
}

void AggContextPeer::copyContext( const Rect& sourceRect,
								  const Rect& destRect,
...
bool AggContextPeer::isMemoryContext()
{
	//LinuxDebugUtils::FunctionNotImplemented(__FUNCTION__);
	return false;
}

bool AggContextPeer::isAntiAliasingOn()
{
	return antiAliasing_;
}

void AggContextPeer::setAntiAliasingOn( bool antiAliasingOn )
{
...
void AggContextPeer::drawThemeSelectionRect( Rect* rect, DrawUIState& state )
{
...
void AggContextPeer::drawThemeFocusRect( Rect* rect, DrawUIState& state )
{
...
void AggContextPeer::drawThemeButtonRect( Rect* rect, ButtonState& state, Rect* captionRect )
{
...
void AggContextPeer::drawThemeButtonFocusRect( Rect* rect )
{
	agg::path_storage path;
	path.move_to( rect->left_, rect->top_ );
	path.line_to( rect->right_, rect->top_ );
...
void AggContextPeer::drawThemeCheckboxRect( Rect* rect, ButtonState& state )
{
	int gcs = context_->saveState();
...
void AggContextPeer::drawThemeRadioButtonRect( Rect* rect, ButtonState& state )
{
	int gcs = context_->saveState();
...
void AggContextPeer::drawThemeComboboxRect( Rect* rect, ButtonState& state )
{
	int gcs = context_->saveState();
...
void AggContextPeer::drawThemeScrollButtonRect( Rect* rect, ScrollBarState& state )
{
	int gcs = context_->saveState();
...
void AggContextPeer::drawThemeDisclosureButton( Rect* rect, DisclosureButtonState& state )
{
	//LinuxDebugUtils::FunctionNotImplemented(__FUNCTION__);
}

void AggContextPeer::drawThemeTab( Rect* rect, TabState& state )
{
	context_->setStrokeWidth(0.5);
...
void AggContextPeer::drawThemeTabPage( Rect* rect, DrawUIState& state )
{
	BackgroundState bkg;
...
void AggContextPeer::drawThemeTabContent( Rect* rect, DrawUIState& state )
{
	BackgroundState bkg;
	bkg.setEnabled( state.isEnabled() );
	bkg.setActive( state.isActive() );
...
void AggContextPeer::drawThemeTabs( Rect* rect, DrawUIState& paneState, TabState& selectedTabState, TabState& otherTabs, const std::vector<String>& tabNames, int selectedTabIndex )
{
	int gcs = context_->saveState();
	...
	Font f = AggGraphicsToolkit::getDefaultFontFor(CONTROLFONT);
	...
	for (size_t i=0; i<tabNames.size(); i ++ ) {
		...
		Size sz = AggGraphicsToolkit::getSize( TABSIZE, tab, &f );
...
void AggContextPeer::drawThemeTickMarks( Rect* rect, SliderState& state )
{
...
void AggContextPeer::drawThemeSlider( Rect* rect, SliderState& state )
{
	//LinuxDebugUtils::FunctionNotImplemented(__FUNCTION__);
}

void AggContextPeer::drawThemeProgress( Rect* rect, ProgressState& state )
{
	int gcs = context_->saveState();
...
void AggContextPeer::drawThemeImage( Rect* rect, Image* image, DrawUIState& state )
{
	//LinuxDebugUtils::FunctionNotImplemented(__FUNCTION__);
}

void AggContextPeer::drawThemeHeader( Rect* rect, ButtonState& state )
{
	Color face = *GraphicsToolkit::getSystemColor( SYSCOLOR_FACE );
	Color shad = *GraphicsToolkit::getSystemColor( SYSCOLOR_SHADOW );
	Color hilite = *GraphicsToolkit::getSystemColor( SYSCOLOR_HIGHLIGHT );
...
void AggContextPeer::drawThemeEdge( Rect* rect, DrawUIState& state, const int32& edgeSides, const int32& edgeStyle )
{
	int gcs = context_->saveState();
...
void AggContextPeer::drawThemeSizeGripper( Rect* rect, DrawUIState& state )
{
	//LinuxDebugUtils::FunctionNotImplemented(__FUNCTION__);
}

void AggContextPeer::drawThemeBackground( Rect* rect, BackgroundState& state )
{
	int gcs = context_->saveState();
...
void AggContextPeer::drawThemeMenuItem( Rect* rect, MenuState& state )
{
	//LinuxDebugUtils::FunctionNotImplemented(__FUNCTION__);
}

void AggContextPeer::drawThemeMenuItemText( Rect* rect, MenuState& state )
{
	//LinuxDebugUtils::FunctionNotImplemented(__FUNCTION__);
}

void AggContextPeer::drawThemeText( Rect* rect, TextState& state )
{
	int gcs = context_->saveState();
	...
	switch ( state.themeFontType_ ) {
		case GraphicsContext::ttMenuItemFont : {
			font = AggGraphicsToolkit::getDefaultFontFor( MENUITEMFONT );
		}
		break;

		case GraphicsContext::ttSelectedMenuItemFont : {
			font = AggGraphicsToolkit::getDefaultFontFor( SELECTEDMENUITEMFONT );
		}
		break;

		case GraphicsContext::ttSystemFont : {
			font = AggGraphicsToolkit::getDefaultFontFor( SYSTEMFONT );
		}
		break;
		...
		case GraphicsContext::ttControlFont : {
			font = AggGraphicsToolkit::getDefaultFontFor( CONTROLFONT );
		}
		break;

		case GraphicsContext::ttMessageFont : {
			font = AggGraphicsToolkit::getDefaultFontFor( MESSAGEFONT );
		}
		break;

		case GraphicsContext::ttToolTipFont : {
			font = AggGraphicsToolkit::getDefaultFontFor( TOOLTIPFONT );
		}
		break;
	}
